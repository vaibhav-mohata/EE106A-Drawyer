...
def rotation_3d(omega, theta):
    ...
    return I + ((omegahat/norm)*np.sin(norm*theta)) + ((np.dot(omegahat, omegahat)/norm**2)*(1-np.cos(norm*theta)))
...
def homog_3d(xi, theta):
    ...
    if (xi[3] == 0) and (xi[4] == 0) and (xi[5] == 0):
    	v_theta = np.array([xi[0]*theta, xi[1]*theta, xi[2]*theta])
    	return np.array([
    		[1, 0, 0, v_theta[0]],
    		[0, 1, 0, v_theta[1]],
    		[0, 0, 1, v_theta[2]],
    	...
    	exp_R = rotation_3d(omega, theta)
    	omegahat = skew_3d(omega)
    	norm = np.linalg.norm(omega)
    	mess_term = (np.dot((np.eye(3) - exp_R), np.dot(omegahat, v)) + np.dot(omega.T, v)*omega*theta) / (norm**2)
    	return np.vstack((np.hstack((exp_R, mess_term.reshape((-1, 1)))), [0, 0, 0, 1]))
...
def prod_exp(xi, theta):
    ...
    for i in range(len(theta)):
    	exp_term = homog_3d(xi[:, i], theta[i])
    	output = np.dot(output, exp_term)
    return output
